...
#Setting Directory:-
setwd("D:/Online_courses/Edwisor/Edwisor/Projects/Santander Customer Transaction")
...
plot1=ggplot(df_train,aes(target))+theme_bw()+geom_bar(stat='count',fill='blue')
plot1
...
missing_val_train<-data.frame(missing_val=apply(df_train,2,function(x){sum(is.na(x))}))
missing_val_train<-sum(missing_val)
missing_val_train

#Finding the missing values in test data
missing_val_test<-data.frame(missing_val=apply(df_test,2,function(x){sum(is.na(x))}))
missing_val_test<-sum(missing_val)
missing_val_test
...
#Confusion matrix
confusionMatrix(cv_predict.rose, target)
